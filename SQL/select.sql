...
("CustomerID","EmployeeID","OrderDate","RequiredDate",
	"ShippedDate","ShipVia","Freight","ShipName","ShipAddress",
	"ShipCity","ShipRegion","ShipPostalCode","ShipCountry")
VALUEs ('BOLID',3,'7/8/1996','8/5/1996','7/15/1996',1,41.34,
...
/*Sort elemnts by name*/
select lower(CompanyName) textdisplay, Country
from Customers
where Country like 'M%';

select count(*) CountofPrdt
from Products
where UnitsInStock>15;/*counts the no of elemnts greater than 10*/

/*Find total cost of unitprice in Products*/

select SUM(UnitPrice) TotalCost
from Products;

/*Current time yyyy-mm-dd hh:mm:ss:millisec*/

select CURRENT_TIMESTAMP today;

/*Dateadd=>add the date from current(or) finds the date after particular time*/

select DATEADD(MONTH,2,2001-10-16);
...
/*ISDATE=> RETURN 1=>VALID DATE, ELSE=>INVALID DATE*/

SELECT ISDATE('2022/12/16') CHK;
SELECT ISDATE('2002/15/32') CHK;

/*MATH*/

SELECT ROUND(SQRT(89),2) ans;

/*TRIM*/

SELECT ltrim('          vshxgvj   ')aftTrim;
SELECT CONCAT('Rajnesh','Durai','R','T')conC;
SELECT SUBSTRING('RAJNESH',2,3) subStr;/*AJN*/

/*CHARINDEX==>tell the index*/
SELECT CHARINDEX('E','HELLO WORLD!')indx;

select REPLACE('Ravi','a','e') revi;