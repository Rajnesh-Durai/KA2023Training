...
/*Assessment page*/
create table Assessment(ID int primary key,AssessmentID nvarchar(25) ,DepartmentID nvarchar(15) foreign key references Department(ID), TopicID int foreign key references Topic(ID),SkillID nvarchar(15) foreign key references Skill(ID),UserID nvarchar(15) foreign key references Users(ID),NumberOfQuestions bigint not null ,DateOfCompletion date not null,"Description" nvarchar(250) null)
...
create table Result(ID nvarchar(25) primary key, AssessmentID int foreign key references Assessment(ID) ,NumberOfTopics int not null,"Status" nvarchar(15) not null,Score Decimal(2,2) not null,CorrectAnswer bigint not null,WrongAnswer bigint not null, SkippedAnswer bigint not null, TotalTime date not null)
